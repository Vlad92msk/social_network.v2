...
export class StoragePluginModule implements IPluginManager<IStoragePlugin>, IPluginExecutor {
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onBeforeSet) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onBeforeSet`, { error })
          throw error
        }
      }
    }
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onAfterSet) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onAfterSet`, { key, error })
          throw error
        }
      }
    }
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onBeforeGet) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onBeforeGet`, { key, error })
          throw error
        }
      }
    }
  ...
  public async executeAfterGet<T>(
    key: StorageKeyType,
    value: T | undefined,
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onAfterGet) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onAfterGet`, { key, error })
          throw error
        }
      }
    }
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onBeforeDelete) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onBeforeDelete`, { key, error })
          throw error
        }
      }
    }
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onAfterDelete) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onAfterDelete`, { key, error })
          throw error
        }
      }
    }
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onKeyTransform?.encode) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} key encode`, { key, error })
          throw error
        }
      }
    }
  ...
  public async executeKeyDecode(key: StorageKeyType): Promise<StorageKeyType> {
    ...
    // Обратите внимание: декодируем в обратном порядке
    ...
    for (const plugin of plugins) {
      if (plugin.onKeyTransform?.decode) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} key decode`, { key, error })
          throw error
        }
      }
    }
    ...
    for (const plugin of this.plugins.values()) {
      if (plugin.onClear) {
        try {
          ...
          this.logger?.error(`Error in plugin ${plugin.name} onClear`, { error })
          throw error
        }
      }
    }
