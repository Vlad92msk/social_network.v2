...
import { authGuard, ownerGuard, roleGuard } from '../guards'
...
export function useGuards() {
  ...
  const createAuthGuard = useCallback(
    (): GuardConfig => ({
      id: 'auth',
      guard: authGuard,
      required: true,
  ...
  const createRoleGuard = useCallback(
    (role: string): GuardConfig => ({
      id: `role_${role}`,
  ...
  const createOwnerGuard = useCallback(
    (userId?: string): GuardConfig => ({
      id: 'owner',
