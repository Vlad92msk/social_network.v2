...
import { FILE_FORMAT_APP, FILE_FORMAT_AUDIO, FILE_FORMAT_IMAGE, FILE_FORMAT_TEXT, FILE_FORMAT_VIDEO } from '../models/fileFormats'

/**
 * Доступные для добавления форматы изображений
 */
export const availableImages = Object.fromEntries(
  Object.entries(FILE_FORMAT_IMAGE).filter(
    ([, fileType]) =>
...
export const availableVideoFormats = Object.fromEntries(
  Object.entries(FILE_FORMAT_VIDEO).filter(([, fileType]) => fileType === FILE_FORMAT_VIDEO.AVI || fileType === FILE_FORMAT_VIDEO.MP4 || fileType === FILE_FORMAT_VIDEO.MPEG),
)
...
export const availableAudioFormats = Object.fromEntries(
  Object.entries(FILE_FORMAT_AUDIO).filter(([, fileType]) => fileType === FILE_FORMAT_AUDIO.WAV || fileType === FILE_FORMAT_AUDIO.WMA || fileType === FILE_FORMAT_AUDIO.WAX),
)
...
export const availableTextFormats = Object.fromEntries(
  Object.entries(FILE_FORMAT_TEXT).filter(
    ([, fileType]) =>
...
export const availableAppFormats = Object.fromEntries(
  Object.entries(FILE_FORMAT_APP).filter(
    ([, fileType]) =>
...
export type AvailableFormats = FILE_FORMAT_APP | FILE_FORMAT_AUDIO | FILE_FORMAT_IMAGE | FILE_FORMAT_TEXT | FILE_FORMAT_VIDEO
...
export type MaterialAttachProps = {
  ...
  availableTypes: (FILE_FORMAT_APP | FILE_FORMAT_AUDIO | FILE_FORMAT_IMAGE | FILE_FORMAT_TEXT | FILE_FORMAT_VIDEO)[]
  getFiles?: (files: any) => void
}

type MaterialAttach = [AddedFile[], (fileInputRef: ChangeEvent<HTMLInputElement>) => void, React.Dispatch<React.SetStateAction<AddedFile[]>>]
...
export const useMaterialsAttach = (props: MaterialAttachProps | undefined): MaterialAttach => {
  ...
  const handleAddFiles = useCallback(
    (fileInputRef: React.ChangeEvent<HTMLInputElement>) => {
      ...
      Array.from(currentFiles)
        .map((file) => {
          ...
          if (!props || (props.availableTypes && !props.availableTypes.includes(type as AvailableFormats)) || (props.maxFileSize && size > fileSizes[props.maxFileSize]))
            return null
