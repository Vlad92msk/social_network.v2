...
@Entity({
  name: 'media',
  comment: 'Общая информация о файле, который пользователь загружает в систему',
})
export class MediaEntity implements MediaItem {
  ...
  @ApiProperty({
    description:
      'В каком разделе пользователь загрузил это медиа (publication/user-about)',
  ...
  @Column({
    type: 'enum',
    enum: MediaEntitySourceType,
    default: MediaEntitySourceType.UNCNOWN,
  })
  ...
  @Column({
    ...
    type: 'varchar',
    length: 50,
    comment: 'Название альбома, если есть',
  })
  album_name: string;
  ...
  @ApiProperty({
    description: 'Мета-информация по файлу',
    type: () => MediaMetadata,
  })
  @OneToOne(() => MediaMetadata, {
    cascade: true,
    onDelete: 'CASCADE',
    eager: true,
  })
  ...
  @ApiProperty({
    description: 'Автор загруженного файла',
    type: () => UserInfo,
  })
  ...
  @ApiProperty({
    description: 'Отмеченные пользователи',
    type: () => [UserInfo],
  })
  @ManyToMany(() => UserInfo)
  @JoinTable({
  ...
  @OneToMany(() => ReactionEntity, (reaction) => reaction.media, {
    cascade: true,
    onDelete: 'CASCADE',
    nullable: true,
  })
  ...
  @ApiProperty({
    description: 'Комментарии к данному медиа',
    type: () => [CommentEntity],
    nullable: true,
  })
  ...
  @ApiProperty({
    description: 'Сообщения, к которым добавлен медиа-файл',
    type: () => [MessageEntity],
    nullable: true,
  })
  ...
  @ApiProperty({
    description:
      'Сообщение, к которому относится данный медиа-файл (аудио сообщение)',
    type: () => MessageEntity,
    nullable: true,
  })
  @ManyToOne(() => MessageEntity, (message) => message.voices, {
    cascade: true,
    onDelete: 'CASCADE',
  })
  ...
  @ApiProperty({
    description:
      'Сообщение, к которому относится данный медиа-файл (видео сообщение)',
    type: () => MessageEntity,
    nullable: true,
  })
  @ManyToOne(() => MessageEntity, (message) => message.videos, {
    cascade: true,
    onDelete: 'CASCADE',
    lazy: true,
  })
  ...
  @ApiProperty({
    description:
      'Пост, к которому относится данный медиа-файл (аудио вложение)',
    type: () => PostEntity,
    nullable: true,
  })
  @ManyToOne(() => PostEntity, (post) => post.voices, {
    cascade: true,
    onDelete: 'CASCADE',
  })
  ...
  @ApiProperty({
    description:
      'Пост, к которому относится данный медиа-файл (видео вложение)',
    type: () => PostEntity,
    nullable: true,
  })
